...
import { Bot, BriefcaseBusiness, Eye, EyeOff, LoaderCircle, Pencil, Plus, Search, ShieldAlert, Trash2, UsersRound, X } from "lucide-react";
...
export function PersonaLibrary({ initialData }: { initialData: LibraryData }) {
  ...
  const filtered = useMemo(() => {
    ...
    return data.personas.filter((persona) => [persona.name, persona.archetype, persona.profile.city, persona.profile.occupation, ...persona.profile.tags].some((value) => value.toLowerCase().includes(normalized)));
  }, [data.personas, query]);
      ...
      {filtered.length ? (
        <div className="persona-library-grid">
          {filtered.map((persona, index) => (
            <article className="persona-library-card" key={persona.publicId}>
              <header><span>{String(index + 1).padStart(2, "0")}</span><div>{persona.visibility === "workspace" ? <Eye size={13} /> : <EyeOff size={13} />}{persona.visibility === "workspace" ? "工作区可见" : "仅自己可见"}</div></header>
              ...
              <footer><span>{persona.source === "manual" ? "手动创建" : "研究生成"}</span>{persona.canEdit ? <button type="button" onClick={() => openEdit(persona)} aria-label={`编辑 ${persona.name}`}><Pencil size={15} /></button> : null}</footer>
            </article>
          ))}
        </div>
