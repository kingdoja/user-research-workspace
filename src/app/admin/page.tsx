  ...
  return (
    <WorkspaceShell viewer={viewer}>
      <div className="workspace-page admin-page">
        ...
        <section className="admin-section"><header><h2>研究项目与报告</h2><span>{studies.length} 个项目</span></header><div className="study-table-wrap"><table className="study-table admin-study-table"><thead><tr><th>项目</th><th>创建者</th><th>工作区</th><th>状态</th><th>报告</th><th>产物</th><th>更新时间</th></tr></thead><tbody>{studies.map((study) => <tr key={study.publicId}><td><strong>{study.title}</strong><small>{studyTypeLabels[study.studyType] ?? study.studyType} · {study.brief}</small></td><td><strong>{study.creatorName}</strong><small>{study.creatorEmail}</small></td><td>{study.workspaceName}</td><td><span className={`study-status status-${study.status}`}>{studyStatusLabels[study.status] ?? study.status}</span></td><td>{study.report ? <a className="admin-report-link" href={`/admin/reports/${study.report.publicId}`}>{study.report.title}</a> : <span className="admin-muted">未生成</span>}</td><td>{study.artifactCount}</td><td>{formatStudyDate(study.updatedAt)}</td></tr>)}</tbody></table></div>{studies.length === 0 ? <p className="admin-empty">还没有研究项目。</p> : null}</section>
