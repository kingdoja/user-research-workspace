...
import { checkRateLimit, internalErrorResponse, isSameOriginRequest, rateLimitResponse } from "@/lib/request-security";
...
export async function POST(request: Request, context: { params: Promise<{ publicId: string }> }) {
  ...
  const rateLimit = await checkRateLimit(request, "agent-message", { limit: 30, windowMs: 15 * 60_000 }, `${viewer.workspaceId}:${viewer.userId}`);
  ...
  try {
    ...
    return internalErrorResponse(error, "Agent 入队失败");
  }
}
