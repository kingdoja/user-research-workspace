...
export async function PATCH(request: Request, context: RouteContext<"/api/platform/collaboration/delegations/[publicId]/status">) {
  ...
  const result = await updateCollaborationDelegationStatus(viewer, publicId, parsed.data.status);
  if (result === "forbidden") return NextResponse.json({ error: "无权处理委托" }, { status: 403 });
  if (result === "not_found") return NextResponse.json({ error: "委托不存在" }, { status: 404 });
