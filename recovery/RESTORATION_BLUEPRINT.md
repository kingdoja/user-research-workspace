...
16. [x] Sandbox Runner 生产资源接入准备包已完成：执行/metrics 凭据分离、Prometheus HTTPS scrape + credentials file、6 条可路由告警、Linux worker 密钥权限/rootless Podman/digest/systemd/探针预检和 operations smoke 均已落地。只有提供真实 worker、DNS/TLS、密钥存储与 Prometheus/Alertmanager 后才能完成首次生产发布。
