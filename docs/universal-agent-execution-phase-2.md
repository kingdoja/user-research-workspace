...
- 失败/阻断 Run 支持安全重试：重试会创建新 Run 并通过 `retry_of_run_id` 保留血缘；如果原 Run 允许副作用，必须重新确认。
- 新增 GitHub Actions 工作流，在 PostgreSQL service 中跑产品工具隔离 smoke、TypeScript、lint 和 Webpack 生产构建。
...
1. 在可控模型环境补充访谈/研究 Worker 完整执行验收。
2. 把 Run 预算默认值接入工作区策略配置，并补充预算消耗告警。
